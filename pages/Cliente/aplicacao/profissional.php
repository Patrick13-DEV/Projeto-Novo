...
if ($categoria !== '') {
    ...
    $sqlTotal = "SELECT COUNT(*) FROM profissionais WHERE especialidade = :especialidade";
                ...
                <?php else: ?>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                        <?php foreach ($profissionais as $profissional): ?>
                            <a
                                href="./agendamento.php?profissional=<?= $profissional['id'] ?>" class="border rounded-xl p-5 flex flex-col hover:shadow-md hover:border-sky-400 transition duration-200">
                                ...
                                <div class="mt-5">
                                    <span
                                        class="block text-center bg-sky-400 text-white rounded-lg py-2 font-poppins text-sm hover:bg-sky-500 transition">
                                        Agendar
                                    </span>
                                </div>
                            </a>
                    ...
                    <?php if ($totalPaginas > 1): ?>
                        <div class="flex justify-center items-center gap-2 mt-10">
                            ...
                            <?php for ($i = 1; $i <= $totalPaginas; $i++): ?>
                                <a
                                    href="?categoria=<?= urlencode($categoria) ?>&pagina=<?= $i ?>" class=" px-4 py-2 rounded-lg font-poppins text-sm transition
                                        <?= $i == $pagina ? 'bg-sky-400 text-white' : 'border hover:bg-sky-100' ?>">
                                    <?= $i ?>
                                </a>
                            <?php endfor; ?>
